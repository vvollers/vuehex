...
When you already own the entire `Uint8Array` (dropped file, clipboard paste, fixture), omit `total-size` and the `updateVirtualData` listener. VueHex detects that
scenario and slices the full array internally while still virtualizing rows.
...
- Self-managed mode activates when neither `total-size` nor `updateVirtualData` is provided.
- You can continue mutating `fullData.value` to show highlights or edits; VueHex re-renders the affected rows.
- Adding `total-size` or an `updateVirtualData` handler later flips the component back to externally managed virtualization.
...
- Return any class name from `cellClassForByte` to highlight ranges, search hits, or diffs. The story uses `story-byte--active` as an example.
...
```ts
import type { VueHexStatusBarLayout } from "@/components/vuehex-api";
